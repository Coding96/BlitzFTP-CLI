...
void ConnectToNetwork()
{
  ...
  else
  {
    ...
    vector<string> Hosts(100);
    
    //start with 100 hosts but can be increased later

    for (int counter = 0; counter < 100; counter++)
    {
      if (counter == 0)
      {
        //if our counter is at 0 then its our first entry
        //which must be our key
        string keyTempVar = createKey();
        Hosts.at(counter) = keyTempVar;
        numOfHosts++;
      }
